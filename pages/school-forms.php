...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'upload' && $auth->hasPermission('upload')) {
        ...
        } elseif (empty($formType) || empty($schoolYear) || empty($title)) {
            setAlert('danger', 'Please fill in all required fields.');
        } elseif (!isset($_FILES['file']) || $_FILES['file']['error'] !== UPLOAD_ERR_OK) {
            setAlert('danger', 'Please select a valid file to upload.');
        } else {
            // Create upload directory
            $uploadDir = UPLOAD_PATH . 'school-forms/' . $schoolYear . '/';
            
            $uploadResult = uploadFile($_FILES['file'], $uploadDir);
            
            if ($uploadResult['success']) {
                $sql = "INSERT INTO school_forms (form_type, school_year, grade_level, quarter, title, description, file_name, original_name, file_path, file_size, uploaded_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                
                $recordId = $db->insert($sql, [
                    $formType, $schoolYear, $gradeLevel, $quarter, $title, $description,
                    $uploadResult['filename'], $uploadResult['original_name'],
                    $uploadResult['path'], $uploadResult['size'], $user['id']
                ]);
                
                if ($recordId) {
                    logActivity($db, $user['id'], 'upload', "Uploaded {$formType} form: {$title}", $recordId);
                    setAlert('success', 'School form uploaded successfully.');
                } else {
                    setAlert('danger', 'Failed to save form to database.');
                }
            } else {
        ...
        header('Location: school-forms.php');
        exit();
    }
}
...
if (isset($_GET['download']) && $auth->hasPermission('view')) {
    ...
    $sql = "SELECT * FROM school_forms WHERE id = ?";
    $record = $db->fetchOne($sql, [$recordId]);
    
    if ($record && file_exists($record['file_path'])) {
        // Log download
        logActivity($db, $user['id'], 'download', "Downloaded {$record['form_type']} form: {$record['title']}", $recordId);
        
        // Log download activity
        $sql = "INSERT INTO download_logs (user_id, record_type, record_id, ip_address) VALUES (?, 'school_form', ?, ?)";
...
$formType = sanitizeInput($_GET['form_type'] ?? '');
...
// Get total count
$countSql = "SELECT COUNT(*) as count FROM school_forms {$whereClause}";
...
<head>
    ...
    <title>School Forms - <?php echo SITE_NAME; ?></title>
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <div class="nav-item">
                <a href="school-forms.php" class="nav-link active">
                    ...
                    <form method="GET" class="row g-3">
                        <div class="col-md-4">
                            ...
                            <input type="text" class="form-control" id="search" name="search" 
                                   placeholder="Title or description" value="<?php echo htmlspecialchars($search); ?>">
                        </div>
                        ...
                        <div class="col-md-2">
                            ...
                            <select class="form-select" id="form_type" name="form_type">
                                ...
                                <option value="SF1" <?php echo $formType === 'SF1' ? 'selected' : ''; ?>>SF1</option>
                                <option value="SF2" <?php echo $formType === 'SF2' ? 'selected' : ''; ?>>SF2</option>
                                <option value="SF5" <?php echo $formType === 'SF5' ? 'selected' : ''; ?>>SF5</option>
                                <option value="SF10" <?php echo $formType === 'SF10' ? 'selected' : ''; ?>>SF10</option>
                                <option value="other" <?php echo $formType === 'other' ? 'selected' : ''; ?>>Other</option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-2">
                            ...
                            <select class="form-select" id="grade_level" name="grade_level">
                                ...
                                <option value="All" <?php echo $gradeLevel === 'All' ? 'selected' : ''; ?>>All</option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-2 d-flex align-items-end">
                            ...
                            <a href="school-forms.php" class="btn btn-outline-secondary">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row g-3">
                            <div class="col-md-3">
                                <label for="form_type_upload" class="form-label">Form Type *</label>
                                <select class="form-select" id="form_type_upload" name="form_type" required>
                            ...
                            <div class="col-md-2">
                                ...
                                <select class="form-select" id="upload_grade_level" name="grade_level">
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">
                        ...
                        School Forms (<?php echo number_format($totalRecords); ?>)
                    </h5>
                ...
                <div class="card-body p-0">
                    <?php if (empty($records)): ?>
                        <div class="text-center py-5">
                            ...
                            <h5 class="mt-3">No forms found</h5>
                                        ...
                                        <tr>
                                            <td>
                                                ...
                                                <?php if ($record['description']): ?>
                                                    <br><small class="text-muted"><?php echo htmlspecialchars($record['description']); ?></small>
                                                <?php endif; ?>
                                            </td>
                                            <td>
                                                <span class="badge bg-primary">
                                                    <?php echo htmlspecialchars($record['form_type']); ?>
                                                </span>
                                            </td>
                                            <td><?php echo htmlspecialchars($record['school_year']); ?></td>
                                            <td><?php echo htmlspecialchars($record['grade_level']); ?></td>
                                            <td>
                                                <?php if ($record['quarter']): ?>
                                                    <span class="badge bg-info"><?php echo htmlspecialchars($record['quarter']); ?></span>
                                                <?php else: ?>
